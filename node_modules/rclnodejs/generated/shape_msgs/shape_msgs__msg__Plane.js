...
const PlaneRefStruct = StructType({
  coef: ArrayType(primitiveTypes.float64, 4),
});
...
class PlaneWrapper {
  ...
  freeze(own = false, checkConsistency = false) {
    for (let i = 0; i < this._wrapperFields.coef.data.length; i++) {
      this._refObject.coef[i] = this._wrapperFields.coef.data[i];
    }
  }
  serialize() {
  ...
  deserialize(refObject) {
    this._refObject.coef = refObject.coef;
    this._wrapperFields.coef.fill(refObject.coef.toArray());
  }
  ...
  static get ROSMessageDef() {
    return {"constants":[],"fields":[{"name":"coef","type":{"isArray":true,"arraySize":4,"isUpperBound":false,"isDynamicArray":false,"isFixedSizeArray":true,"pkgName":null,"type":"float64","stringUpperBound":null,"isPrimitiveType":true},"default_value":null}],"baseType":{"pkgName":"shape_msgs","type":"Plane","stringUpperBound":null,"isPrimitiveType":false},"msgName":"Plane"};
  }
  hasMember(name) {
...
class PlaneArrayWrapper {
  ...
  fill(values) {
    ...
    values.forEach((value, index) => {
      if (value instanceof PlaneWrapper) {
        this._wrappers[index].copy(value);
      } else {
        this._wrappers[index] = new PlaneWrapper(value);
      }
    });
  }
